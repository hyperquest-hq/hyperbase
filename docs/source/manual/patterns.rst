...
.. note::
   To simplify the notation, hyperbase applies the convention that an atom without at type annotation defaults to the conjunction type (``/J``). This means that any edge with a functional pattern connector can contain arguments of any other type and remain itself valid within the Semantic Hypergraph representation.
