...
Derives a taxonomy from concepts defined with builders or modifiers. For example, `(of/Br.ma founder/Cc.s psychoanalysis/Cc.s)` is a type of `founder/Cc.s`, so the following hyperedge is added:
...
We define actors as specific entities that are capable of acting in some sense. This simple command identifies hyperedges corresponding to actors by applying the following criteria:
...
This command traverses the entire hypergraph to identify actors, and then adds hyperedges like the following:
...
2. The deep predicate atom of the predicate hyperedge has a lemma belonging to a predetermined list of verb lemmas that denote a claim (e.g.: "say", "claim").
...
More specifically, conflicts are detected according to the following criteria:

1. Hyperedge is a relation with predicate of type `Pd`.
2. The deep predicate atom of the predicate hyperedge has a lemma belonging to a predetermined list of verb lemmas that denote an expression of conflict (e.g.: "warn", "kill").
